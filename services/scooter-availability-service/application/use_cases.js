...
class ScooterAvailabilityUseCases {
  ...
  async checkAvailableScooters({ lat, lon, radiusMeters }) {
    ...
    const scooters = await this.repository.findAvailableNear(lat, lon, radiusMeters);
    this.logger.info("check_available_scooters", { count: scooters.length, radiusMeters });
    return scooters;
  }
  ...
  async updateScooterStatus({ scooterId, status, batteryLevel, location }) {
    ...
    if (!location || !Number.isFinite(location.lat) || !Number.isFinite(location.lon)) {
      throw new ValidationError("Location must include numeric lat and lon values.");
    }
